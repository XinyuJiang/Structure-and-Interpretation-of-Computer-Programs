...
def closer_city(lat, lon, city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    goal=make_city('shanghai',lat,lon)
    if distance(goal,city1)<distance(goal,city2):
...
def create_board(rows, columns):
    ...
    board=[]
    for i in range (rows):
        board.append(['-' for i in range(columns)])
    return board 
    #return _______
...
def put_piece(board, max_rows, column, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    while board[max_rows-1][column]!='-' and max_rows>-1:
        max_rows=max_rows-1
    if max_rows ==-1:
        return (-1,board)
    row=max_rows
    #print ("board:",board)
    newrow=replace_elem(board[row-1],column,player)
    newboard=replace_elem(board,row-1,newrow)
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    if col<0:
        return (-1,board)
    while board[max_rows-1][col]!='-' and max_rows>-1:
        max_rows=max_rows-1
    if max_rows ==-1:
        return (-1,board)
    row=max_rows
    #print ("board:",board)
    newrow=replace_elem(board[row-1],col,player)
    newboard=replace_elem(board,row-1,newrow)
    #print ("newboard:",newboard)
    tup=(row-1,newboard)
    #print ("tup:",tup)
