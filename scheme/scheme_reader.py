...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 2
            src.remove_front()            
            #can't use val = src.current() here.
            val = scheme_read(src)
            #read_tail return the rest part of the list
            ...
            #it means src is at the beginning of an expression
            #print ("#scheme_read:",scheme_read(src))
            #notice that if src is a '(' ,then when excute to read_tail, the src will already jump over the whole expression. Because of recursion call.
            return Pair(scheme_read(src), read_tail(src))
            #print ("#src.current:",src.current())
            # END PROBLEM 1
    except EOFError:
        raise SyntaxError('unexpected end of file')
