...
def eval_all(expressions, env):
    ...
    # BEGIN PROBLEM 8
    if expressions!= nil:
        #expression not equal to nil
        rest = expressions.second
        #judge whether the rest is nil
        if rest == nil:
            #if the rest equal to nil, then do not move the next layer
...
class Frame:
    ...
    def define(self, symbol, value):
        ...
        # BEGIN PROBLEM 3
        # takes a symbol (represented by a Python string) and value and binds the value to that symbol in the frame.
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        #evaluate the operands
        rest = operands.map(lambda item : scheme_eval(item,env))
        return scheme_apply(self,rest,env)
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try :
            #Call self.fn on all of those arguments.
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        #binding target and value and return target(noticed that we should use expressions.second.first not expressions.second, because expressions.second is a pair)
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        ...
        #if use scheme_eval on the same object twice last will be incorrect.
        if not scheme_truep(last):
            return False
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        symbol = Pair(current.first,symbol)
        expression = Pair(scheme_eval(current.second.first,env),expression)
        #print("@symbol:",symbol)
        #print("@expression",expression)
        bindings = bindings.second
    let_env = env.make_child_frame(symbol,expression)
