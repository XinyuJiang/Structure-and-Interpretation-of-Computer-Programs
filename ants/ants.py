...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        #if insect is ant and not true queenant


        if insect.is_ant:
            # Phase 6: Special Handling for BodyguardAnt and QueenAnt
            if isinstance(insect,QueenAnt) and insect.count == 1:
                return 
...
#    Queen_power=[]
    ...
    def can_contain(self, other):
        # BEGIN Problem 11
        return (self.container == True and other.container == False and self.ant == None)
        # END Problem 11
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        tempdist = 0
        while temp != hive :
            if len(temp.bees)!=0 and tempdist >= self.min_range and tempdist<=self.max_range: 
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 3
        #???
        #why self.add_insect(insect) doesn't work?
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount>=self.armor: 
            """this place use copy to avoid list change while loop.
            according reduce_armor and remove function,
            Although bee changed, self.place.bees[:] will not change."""
...
class LongThrower(ThrowerAnt):
    ...
    name = 'Long'
    food_cost = 2
    #armor = 1 no use. because in insect it already initialize the armor as 1
    # BEGIN Problem 6
    implemented = True   # Change to True to view in the GUI
    # END Problem 6
...
class WallAnt(Ant):
    name = 'Wall'
    food_cost = 4
    #armor = 4 doesn't wokr. Everytime you just change a particular instane, not works on every other instance.
    #so for the attribute in class, not in __init__, we can change it directly.but we can only change __init__ by __init__
    implemented = True   # Change to True to view in the GUI
    def __init__(self, armor=4):
        """Create an Ant with an ARMOR quantity."""
        Insect.__init__(self, armor)#Insect or Ant both works well.
# END Problem 7
...
class HungryAnt(Ant):
    ...
    # END Problem 10
    digesting = 0
    #__init__ be used to store something that should be initialize everytime we creat an instance.Here put digesting in class or __init__ are both well.
    def __init__(self,armor = 1):
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        Insect.__init__(self, armor)#Insect or Ant both works well.because here already exist an __init__ so we need to initialize the armor.
...
# END Problem 13
    ...
    def __init__(self):
        ...
        if self.tag == 0:
            self.count = 1
            QueenAnt.tag = 1#this way to change the class attribute
        else :
            self.count = 2
        Insect.__init__(self, 1)#Insect or Ant both works well.because here already exist an __init__ so we need to initialize the armor.
        # END Problem 13
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        stor = []
        #store ant's position which behind queen
        if self.count == 1:
            ScubaThrower.action(self, colony)
            temp = self.place
            while temp.exit!= None :
                if temp.exit.ant!=None:
                    if temp.exit.ant.buff == False:  
                        stor.append(temp.exit)
        #consider the case that have container
                    if  isinstance(temp.exit.ant,BodyguardAnt) or isinstance(temp.exit.ant,TankAnt):
                        if temp.exit.ant.ant != None:
                            if temp.exit.ant.ant.buff == False and temp.exit.ant.buff == True: 
                                stor.append(temp.exit)
                temp = temp.exit
        #every ant's damage *2 
            for temp2 in stor:
                if isinstance(temp2.ant,BodyguardAnt) or isinstance(temp2.ant,TankAnt):
                    if isinstance(temp2.ant,TankAnt) and temp2.ant.buff == False:
                        temp2.ant.damage *= 2
                        temp2.ant.buff = True
                    if temp2.ant.ant != None and temp2.ant.ant.buff == False:
                        temp2.ant.ant.damage *= 2#change the damage of the ant which inside a container
                        temp2.ant.ant.buff = True
                else:
                    temp2.ant.damage *= 2
                    temp2.ant.buff = True
        else :
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor == 0 and self.count == 1:
            bees_win()
        if self.armor == 0 and self.count>1:
            self.place.remove_insect(self)
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    count = 0
    def effecttime(colony):
        nonlocal count
        if count < duration:
            count += 1
            return effect(tempaction)(colony)
        else :
            return tempaction(colony)
    bee.action = effecttime
    # END Problem EC
